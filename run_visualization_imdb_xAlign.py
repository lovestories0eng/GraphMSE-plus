...
def test(model, batch_size=200, test_method="best_val"):
    ...
    for type in type_set:
        ...
        one_hot[type] = []
        for i, metapath in enumerate(metapath_set[type]):
            metapath_label[type][metapath] = torch.zeros(batch_size, data.type_num)
    ...
    with torch.no_grad():
        ...
        while batch < len(test_index):
            ...
            batch_test_logits_dict, path_vec = model(batch_test_rows_dict, batch_test_feature_dict, True)  # 获取模型的输出
            for type in batch_test_logits_dict:
                ...
                y_pred.extend(batch_test_logits_dict[type].max(1)[1].cpu().numpy().tolist())  # 预测标签：对预测结果按行取argmax
                y_true.extend(batch_test_label_dict[type].cpu().numpy().tolist())  # 计算在测试节点/数据上的准确率
            batch += batch_size
