...
def train(model, epochs, method="all_node", ablation="all"):
    ...
    with open("result/"+ dataset + "_time.txt",
              "a") as f:
        f.write("Pretrain:" + str(pretrain_convergence) + "\tSelect:" + str(select_convergence) + "\n")
...
def test(model, batch_size=200, test_method="best_val"):
    ...
    with torch.no_grad():
        ...
        if shuffle == True:
            ...
            feature_mode = ""
        with open("result/"+dataset + "_" + str(train_percent) + "_" + test_method + "_" + feature_mode + ablation + ".txt",
                  "a") as f:
...
if __name__ == '__main__':
    ...
    single_path_limit = 10  # lambda = 5

    # Automatically calculated parameters
    num_batch_per_epoch = 5  # 每个epoch循环的批次数
    batch_size = train_percent//20 * 128
