...
def test(model, batch_size=200, test_method="best_val"):
    ...
    def index_to_feature_wrapper(dict):
        return type_list.index_to_features(dict, data.x, select_method)
...
if __name__ == '__main__':
    ...
    learning_rate = 0.01  #
    select_method = "end_node"  # Only used in end-node study
    ...
    select_model = GraphMSE(metapath_list=metapath_name, input_dim=input_dim, pre_embed_dim=pre_embed_dim,
                            select_dim=output_dim, mlp_settings=mlp_settings, endnode_test=True).to(DEVICE)
