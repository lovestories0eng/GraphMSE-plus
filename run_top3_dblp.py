...
def test(model, batch_size=200, test_method="best_val"):
    ...
    with torch.no_grad():
        ...
        while batch < len(test_index):
            ...
            batch += batch_size
            batch_test_logits_dict, _ = model(batch_test_rows_dict, batch_test_feature_dict)
...
if __name__ == '__main__':
    ...
    metapath_name = {'A': ['AP', 'APC','APA']}
    metapath_list = [[1,2],[0,1]]
    print("Selected Path: AP, APA, APC")
    ...
    train(model=select_model, epochs=30, method="all_node", ablation=ablation)
